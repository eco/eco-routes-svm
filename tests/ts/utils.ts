...
import { EVM_DOMAIN_ID, IGP_PDA, IGP_PROGRAM_ID, OVERHEAD_IGP, USDC_DECIMALS } from "./constants";
...
export function generateIntentHash(route: Route, reward: Reward): Uint8Array {
    const parts: Uint8Array[] = [
        ...
        ...route.tokens.flatMap((tokenAmount) => [hexToBytes(tokenAmount.token), bn(tokenAmount.amount)]),
        ...route.calls,

        reward.creator,
        reward.prover,
        ...reward.tokens.flatMap((tokenAmount) => [hexToBytes(tokenAmount.token), bn(tokenAmount.amount)]),
...
export function calcProgramHash(route: EvmRoute, reward: EvmReward): Uint8Array {
...
export const buildPayForGasIx = (solverPubkey: PublicKey, dispatchedMessageBytes: Buffer, uniqueMessage: PublicKey): TransactionInstruction => {
    ...
    const [programDataPda] = PublicKey.findProgramAddressSync(
        [anchor.utils.bytes.utf8.encode("hyperlane_igp"), anchor.utils.bytes.utf8.encode("-"), anchor.utils.bytes.utf8.encode("program_data")],
        IGP_PROGRAM_ID
    );
    const [gasPaymentPda] = PublicKey.findProgramAddressSync(gasPaymentSeeds(uniqueMessage), IGP_PROGRAM_ID);

    // 600k should be sufficient for the test for gas amount,
    const GAS_AMOUNT = BigInt(600_000);

    const data = encodePayForGasData(dispatchedMessageBytes, EVM_DOMAIN_ID, GAS_AMOUNT);
...
class SerializableAccountMeta {
    ...
    constructor(k: { pubkey: Uint8Array; isSigner: boolean; isWritable: boolean }) {
