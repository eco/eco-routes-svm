...
const salt = (() => {
	const bytes = anchorUtils.bytes.utf8.encode("svm-evm-e2e-test-abcd".padEnd(32, "\0"));
...
describe("SVM -> EVM e2e", () => {
	...
	before("prepares intent data", async () => {
		...
		console.log("SVM inbox bytes:", hex32ToNums(addressToBytes32Hex(INBOX_ADDRESS)));
	...
	it("Fulfill intent on EVM", async () => {
		...
		console.log("Solver USDC balance:", ethers.formatUnits(solverUsdcBalance, 6));
		...
		async function diagnosePotentialRevert() {
			try {
				...
				console.log("callStatic succeeded (on-chain revert is gas/fee related)");
			} catch (err: any) {
