...
const salt = (() => {
	const bytes = anchorUtils.bytes.utf8.encode("evm-svm-e2etes1284wa".padEnd(32, "\0"));
...
describe("EVM → SVM e2e", () => {
	...
	it("publishes & funds an intent on EVM", async () => {
		...
		hyperProver = HyperProver__factory.connect(HYPER_PROVER_ADDRESS, creatorEvm);
		...
		const usdcApproveTx = await usdc
			.connect(creatorEvm)
			.approve(INTENT_SOURCE_ADDRESS, usdcAmount(1));
	...
	it("fulfills intent on Solana", async () => {
		...
		console.log("Fulfill tx: ", fulfillIx.keys);
		...
		try {
			...
			console.log("msg ID sent: ", Buffer.from(dispatchedMessagePda.toBytes()).toString("hex"));
		} catch (error) {
		...
		try {
			...
			console.log("Dispatched message account data length:", dispatchedMsgAccountInfo.data.length);

			const dispatchedMsgBytes = dispatchedMsgAccountInfo.data.slice(
				DISPATCHED_MSG_PDA_HEADER_LEN + 1
			);

			console.log("enc[0..9] =", Buffer.from(dispatchedMsgBytes.slice(0, 9)).toString("hex"));
