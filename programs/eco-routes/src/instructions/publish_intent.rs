...
pub fn publish_intent(ctx: Context<PublishIntent>, args: PublishIntentArgs) -> Result<()> {
    ...
    let route = Route::new(salt, destination_domain_id, inbox, route_tokens, calls)?;
    ...
    *ctx.accounts.intent =
        Intent::new(intent_hash, route.clone(), reward.clone(), ctx.bumps.intent)?;

    emit!(events::IntentCreated::new(intent_hash, route, reward));
