...
/// `fulfill` requires `route.deadline >= now`, so past the deadline no second
/// fulfill can land whether or not the marker exists. It says nothing about
/// whether the reward has been collected, and there is no deadline that does:
/// `withdraw` is not time-gated at all, and `refund` refuses outright while a
/// `Proof` exists (`IntentFulfilledAndNotWithdrawn`), so a proven intent stays
/// claimable indefinitely. Passing `reward.deadline` makes the reward
/// *refundable*, not refunded — the creator may never call `refund`, and the
/// solver can still prove and withdraw until they do.
///
