...
fn refund_intent_native_success() {
    ...
    let intent_hash = intent_hash(&intent.destination_chain, &route_hash, &intent.reward);
    ...
    let result = ctx.refund_intent(&intent, vault, route_hash, proof, creator, vec![]);
...
fn refund_intent_tokens_success() {
    ...
    let token_accounts: Vec<_> = intent
        ...
        .collect();
    let result = ctx.refund_intent(&intent, vault, route_hash, proof, creator, token_accounts);
    assert!(result.is_ok_and(common::contains_event(IntentRefunded::new(
        intent_hash,
        creator,
    ))));
...
fn refund_intent_fulfilled_on_wrong_chain_success() {
    ...
    let intent_hash = intent_hash(&intent.destination_chain, &route_hash, &intent.reward);
    let vault = state::vault_pda(&intent_hash).0;
    let proof_pda = Proof::pda(&intent_hash, &intent.reward.prover).0;

    let fulfillment_proof = Proof::new(random::<[u8; 32]>().into(), Pubkey::new_unique());
    ctx.set_proof(proof_pda, fulfillment_proof);
    ctx.expire_intent(&intent);

    let result = ctx.refund_intent(&intent, vault, route_hash, proof_pda, creator, vec![]);
    assert!(result.is_ok_and(common::contains_event(IntentRefunded::new(
        intent_hash,
        creator,
    ))));
...
fn refund_intent_invalid_creator_fail() {
    ...
    let result = ctx.refund_intent(&intent, vault, route_hash, proof, wrong_creator, vec![]);
...
fn refund_intent_invalid_vault_fail() {
    ...
    let result = ctx.refund_intent(&intent, wrong_vault, route_hash, proof, creator, vec![]);
    assert!(result.is_err_and(common::is_portal_error(
...
fn refund_intent_invalid_proof_fail() {
    ...
    let result = ctx.refund_intent(&intent, vault, route_hash, wrong_proof, creator, vec![]);
    assert!(result.is_err_and(common::is_portal_error(
