...
fn atomic_fulfill_prove_pay_for_gas() {
    ...
    report_tx_metrics("ATOMIC (no tokens, no calls)", tx_size, num_accounts, num_signatures, tx.compute_units_consumed);
}

/// Same as atomic_fulfill_prove_pay_for_gas but with 1 route token and
/// 1 route call (SPL Token transfer_checked). This is the typical production
/// case and tests whether it still fits in a single transaction.
#[test]
fn atomic_with_token_and_call() {
    let mut ctx = common::Context::default();
    ...
    // Verify the token transfer happened
    assert_eq!(ctx.token_balance_ata(&mint, &recipient), amount);

    report_tx_metrics("ATOMIC (1 token + 1 call)", tx_size, num_accounts, num_signatures, tx.compute_units_consumed);
}

fn report_tx_metrics(label: &str, tx_size: usize, num_accounts: usize, num_sigs: usize, cu: u64) {
    // Estimate v0+ALT size
    let num_in_keys = num_sigs + 3; // signers + 3 dynamic PDAs
    let num_in_alt = num_accounts.saturating_sub(num_in_keys);
    let v0_size = 1 + num_sigs * 64  // signatures
        + 1                           // msg prefix
        + 3                           // header
        + 1 + num_in_keys * 32       // account keys
        + 32                          // blockhash
        + 1                           // ix count
        + (tx_size - (1 + num_sigs * 64 + 3 + 1 + num_accounts * 32 + 32 + 1)) // ix data
        + 1 + 32 + 2 + num_in_alt;  // ALT section

    eprintln!("=== {} ===", label);
    eprintln!("  Legacy size:     {} / 1232 bytes", tx_size);
    eprintln!("  v0+ALT size:     ~{} / 1232 bytes (est.)", v0_size);
    eprintln!("  v0+ALT headroom: ~{} bytes", 1232_i64 - v0_size as i64);
    eprintln!("  Accounts:        {} ({} in keys, {} in ALT)", num_accounts, num_in_keys, num_in_alt);
    eprintln!("  Signatures:      {}", num_sigs);
    eprintln!("  Compute units:   {} / 1,400,000 ({:.1}%)", cu, cu as f64 / 14_000.0);
}
