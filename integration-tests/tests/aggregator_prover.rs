...
use aggregator_prover::instructions::{AggregatorProverError, IntentPreimage};
use aggregator_prover::state::{Config, ProofAccount, MAX_MEMBERS};
use anchor_lang::error::ErrorCode;
use anchor_lang::{AnchorDeserialize, InstructionData, ToAccountMetas};
use anchor_spl::associated_token::get_associated_token_address_with_program_id;
use eco_svm_std::prover::{IntentHashClaimant, IntentProven, Proof, ProofData, ProveArgs};
...
fn args(claimant: Pubkey) -> ProveArgs {
    ...
    ProveArgs {
        ...
        data: anchor_lang::prelude::borsh::to_vec(&vec![preimage]).unwrap(),
    }
}
...
fn aggregate_address(args: &ProveArgs) -> Pubkey {
    Proof::pda(
...
#[test]
fn prove_selects_first_member_and_emits_standard_event() {
    ...
    let result = context.aggregator_prover().prove(args, &MEMBERS);
    assert!(
...
#[test]
fn prove_falls_back_to_second_member() {
    let mut context = initialized();
    let claimant = Pubkey::new_unique();
    let args = args(claimant);
    set_member_proof(&mut context, &args, MEMBERS[1], DESTINATION, claimant);
    assert!(context.aggregator_prover().prove(args, &MEMBERS).is_ok());
}

#[test]
fn prove_wrong_destination_does_not_shadow_valid_member() {
    let mut context = initialized();
    let claimant = Pubkey::new_unique();
    let args = args(claimant);
    set_member_proof(
...
#[test]
fn prove_skips_zero_claimants_malformed_data_and_wrong_owners() {
    for variant in 0..4 {
        let mut context = initialized();
        let claimant = Pubkey::new_unique();
        let args = args(claimant);
...
#[test]
fn prove_cannot_omit_reorder_or_substitute_members() {
    for members in [
        vec![MEMBERS[1]],
...
#[test]
fn prove_rejects_unproven_and_wrong_destination_only() {
...
#[test]
fn prove_rejects_forged_destination_preimage_and_claimant() {
    for variant in 0..4 {
        let mut context = initialized();
        let claimant = Pubkey::new_unique();
...
#[test]
fn prove_rejects_invalid_data() {
    for data in [vec![], vec![0; 4], vec![255; 4]] {
        let mut context = initialized();
...
#[test]
fn prove_repeated_proof_is_idempotent_but_cannot_change_claimant() {
    let mut context = initialized();
    let claimant = Pubkey::new_unique();
    set_member_proof(
        &mut context,
    ...
    assert!(context
        .aggregator_prover()
        .prove(args(other_claimant), &MEMBERS)
        .is_err_and(common::is_error(AggregatorProverError::IntentAlreadyProven)));
}

#[test]
fn prove_batches_are_atomic() {
    let mut context = initialized();
    let claimant = Pubkey::new_unique();
    let mut args = args(claimant);
    let proof_address = aggregate_address(&args);
    set_member_proof(&mut context, &args, MEMBERS[0], DESTINATION, claimant);
    let mut preimages = Vec::<IntentPreimage>::try_from_slice(&args.data).unwrap();
    ...
    args.data = anchor_lang::prelude::borsh::to_vec(&preimages).unwrap();
    assert!(context
        .aggregator_prover()
        .prove(args, &MEMBERS)
        .is_err_and(common::is_error(AggregatorProverError::NoMatchingProof)));
...
fn close_proof_rejects_unscoped_signer() {
    ...
    let proof_address = aggregate_address(&args);
    set_member_proof(&mut context, &args, MEMBERS[0], DESTINATION, claimant);
    context.aggregator_prover().prove(args, &MEMBERS).unwrap();
...
fn existing_portal_withdraws_native_and_tokens_and_closes_only_aggregate_proof() {
    for token_2022 in [false, true] {
        ...
        let args = ProveArgs {
            ...
            data: anchor_lang::prelude::borsh::to_vec(&vec![IntentPreimage {
                route_hash,
...
fn max_members_can_resolve_last_proof() {
    ...
    let result = context.aggregator_prover().prove(args, &members).unwrap();
...
#[test]
fn prove_rejects_substituted_aggregate_pda() {
    let mut context = initialized();
    let claimant = Pubkey::new_unique();
    let args = args(claimant);
    set_member_proof(&mut context, &args, MEMBERS[0], DESTINATION, claimant);
    let mut instruction = context
        .aggregator_prover()
        .build_prove_instruction(args, &MEMBERS);
    instruction.accounts[6].pubkey = Pubkey::new_unique();
    assert!(context
        .aggregator_prover()
        .send_instruction(instruction)
        .is_err_and(common::is_error(AggregatorProverError::InvalidProof)));
...
#[test]
fn prove_batch_resolves_members_independently() {
    let mut context = initialized();
    let claimant = Pubkey::new_unique();
    let other_claimant = Pubkey::new_unique();
    let mut args = args(claimant);
...
fn existing_portal_prove_dispatches_member_then_aggregator() {
    ...
    context
        ...
        .prove_intent_via_local_prover(
            ...
            CHAIN_ID,
            vec![marker],
            portal::state::dispatcher_pda(&local_prover::ID).0,
            vec![member_proof],
        )
        .unwrap();
    let preimages = vec![IntentPreimage {
        route_hash: intent.route.hash(),
        reward_hash: intent.reward_hash,
    }];
    let data = anchor_lang::prelude::borsh::to_vec(&preimages).unwrap();
    let args = ProveArgs {
        domain_id: CHAIN_ID,
        proof_data: ProofData::new(
            CHAIN_ID,
    ...
    let proof = context
        .account::<ProofAccount>(&Proof::pda(&intent.intent_hash, &aggregator_prover::ID).0)
        .unwrap();
