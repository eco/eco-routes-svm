...
- **`prover::prove` CPI** (`packages/eco-svm-std/src/prover.rs`) builds local-prover's fixed six-account layout, signing `caller` via `caller_seeds`. Flash-Fulfiller uses its prover-scoped `prove_authority_pda`. Portal's `prove` assembles a variable account list and signs its prover-scoped `dispatcher`; this also supports aggregator-prover's additional Config and member proof accounts.
