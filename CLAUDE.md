...
- **aggregator-prover** — source-chain `aggregate(AggregateArgs)` creates the existing `Proof` PDA consumed by unchanged Portal withdrawal/refund logic. There is no `prove` instruction: destination relay and source delivery use concrete member provers. A singleton `Config` holds 1–8 ordered executable member IDs, initialized once by the program's upgrade authority. Permissionless `aggregate` verifies every member's canonical proof PDA, ownership, discriminator, destination, and nonzero claimant, selecting the first matching proof. `AggregateArgs` contains `proof_data` and typed `preimages: Vec<IntentPreimage { route_hash, reward_hash }>` binding each intent hash to the destination. It creates its own standard proof and emits `IntentProven`; priority is evaluated at creation, and later conflicting proofs cannot overwrite it. No challenges or bridge dispatch. **A member proof alone does not block Portal refunds**: solver integration must aggregate before the reward becomes refundable (ideally with inbound delivery) and withdraw through the aggregator. `close_proof` accepts only Portal's signer scoped to the aggregator and closes only the aggregate proof. Member proofs remain owned by their provers. Every distinct member set requires its own program deployment and initialization before use; membership has no setter or close instruction. Details and account ordering: README's Aggregator-Prover section.
...
- **`prover::prove` CPI** (`packages/eco-svm-std/src/prover.rs`) builds local-prover's fixed six-account layout, signing `caller` via `caller_seeds`. Flash-Fulfiller uses its prover-scoped `prove_authority_pda`. Portal's `prove` assembles a variable account list and signs its prover-scoped `dispatcher` to a concrete member prover. Aggregator's source-chain `aggregate` is called separately, not through Portal's `prove` dispatch.
