...
Chainer transport tests also require the non-Anchor, test-only `template-buffer` (never deploy it):
...
The optional chainer `resource-metrics` feature is for local SBF heap profiling only; leave it disabled in release builds.

The chainer's production staged-order transport is `init_order_buffer` (first chunk)
→ `write_order_buffer` (append-only chunks, as needed) → `seal_order_buffer` (full
validation + OrderAnnounced self-CPI) → `chain_from_account` (read-only buffer, no
buffer-authority signer). `close_order_buffer` returns rent separately to its original
authority, including abandoned buffers. The buffer is transport only: the shared chain
handler recomputes the Order hash and validates the order-specific escrow before moving
value. Never add a time gate or an execute-and-close authority tail. Native packet and
stock-heap tests live in `chain_nested`; the generic test-only buffer above is retained
for inline ABI/CPI regressions, not as a production dependency.
...
- **intent-chainer** — publishes an intent whose reward amount is only known once an earlier intent executes (`chain`, plus a permissionless `announce_order` that records the complete nested preimage as an Anchor self-CPI event). Measures its per-order escrow ATA, renders typed amounts and EVM/TRON/Solana remote vault recipients into intent2's route data, pushes the measured balance straight into `vault_pda(intent_hash)`, and optionally CPIs `portal::publish`. The SVM counterpart of eco-routes' `IntentChainer` (eco-routes#438), but **necessarily a separate transaction**, not a route call of intent1: intent2's vault is a PDA of the intent hash, which depends on the amount, and Solana requires every account to be named before the transaction runs. `portal::fulfill → chain → portal::publish` is additionally rejected as `ReentrancyNotAllowed` (pinned by `chain_cannot_publish_from_inside_a_route_call`). Because the balance is therefore at rest and `chain` is permissionless, **custody is seeded by `keccak(borsh(order))`** — `escrow_authority_pda(order_commitment)`. That is a security boundary: it is what makes intent1's hash-committed route the authorization anchor transitively, and collapsing it to anything the order does not determine lets the first caller sweep the escrow into an order of their own authorship. **`Order.portal` is a committed field, so one deployed chainer serves any number of portal deployments** — the PDA derivations re-implement portal's seeds against the supplied program rather than calling `portal::state::vault_pda`, which would bind to `portal::ID` at compile time. Committed rather than caller-supplied is a security boundary: the vault derives under that portal, so a free parameter would let a caller point the sweep at a program they wrote. It never calls `portal::fund` — `withdraw` pays from live vault balances with no funded flag — and it deliberately does **not** consult the child's `WithdrawnMarker`. The EVM chainer *does* check Portal status there; **do not port that check back**, and note the reason is not a difference in refund rules (those are the same shape on both chains). It is that on EVM `chain()` is the parent's last route call, so a revert unwinds the swap and strands nothing, whereas here the balance is already at rest in `escrow_authority_pda` and this instruction is its only exit — no sweep, no recovery instruction, no way to clear a marker. Any check that can reject an otherwise-fundable state therefore makes a recoverable balance unrecoverable. The marker is also nobody's to avoid, since `withdraw` marks even when it pays zero against an empty vault, and the push does land somewhere recoverable: `refund` returns `Ok` immediately once a marker exists, paying live vault balances to `reward.creator`. Existing vault balances of any size are allowed; `PushShortfall` checks that the current transfer increases the vault balance by exactly the measured input. **Fresh child route salts belong to the builder**: reusing all rendered child fields and the measured amount addresses the same intent, including any delayed proof after refund. Neither a different parent nor a different Order commitment guarantees child uniqueness. A vault balance is not a freshness record; no child-consumption registry is added. Remote recipients are data, never local payees; every nested template/configuration is part of the Order commitment. `announce_order` requires the event authority/program accounts and indexers must read inner instructions. See `programs/intent-chainer/README.md` for the Borsh ABI, u128 arithmetic, 2 KiB aggregate render / 4 KiB order limits, and packet-checked staging requirements.
