...
Seven production on-chain programs (plus the localnet-only `dummy-ism`, `mock-polymer-prover`, `malicious-prover`, and `malicious-proof-closer` test programs) and one shared crate. `eco-svm-std` (`packages/eco-svm-std`) defines `Bytes32`, `Proof`/`ProofData`/`ProveArgs`, and the generic `prover::prove` CPI helper used by local-prover callers. Other prover account layouts are assembled by Portal's variable-account dispatch.
...
Polymer is pull-based — the bridge never calls us:

```
  Relayer ──create_accounts / load_proof──► Polymer program          (stage the proof)
     └─────► PolymerProver.validate ──CPI validate_event──► Polymer program
                    └─► Proof PDA ────────────────────────► Portal.withdraw   (later, by anyone)

  Portal ──prove (CPI dispatch)──► PolymerProver ──`Prove: program: <id>, <hex>` log──► EVM PolymerProver.validateSolana
```

### Programs (`programs/*`)

- **aggregator-prover** — source-chain `aggregate(intent_hash)` creates the existing `Proof` PDA consumed by unchanged Portal withdrawal/refund logic. There is no `prove` instruction: destination relay and source delivery use concrete provers. A singleton `Config` holds 1–8 configured executable prover IDs, initialized once by the program's upgrade authority. `init()` takes no instruction arguments; it validates and stores the prover keys from `remaining_accounts` in their supplied order. Permissionless `aggregate` accepts a caller-selected prover and its proof as named accounts. It checks that the prover is configured and executable, and verifies its proof’s canonical PDA, ownership, discriminator, data shape, and nonzero claimant. No other prover accounts are required, and config order has no priority semantics. `aggregate` handles exactly one intent per call. Its only argument is `intent_hash`; it copies the selected prover’s destination and claimant unchanged. Destination correctness is trusted to configured provers, with no preimage validation or challenge recovery. It creates its own standard proof and emits `IntentProven`; it always calls `.init()`, so an existing proof PDA rejects every subsequent aggregation, including identical proofs. No challenges or bridge dispatch. **A configured prover’s proof alone does not block Portal refunds**: solver integration must aggregate before the reward becomes refundable (ideally with inbound delivery) and withdraw through the aggregator. `close_proof` accepts only Portal's signer scoped to the aggregator and closes only the aggregate proof. Underlying proofs remain owned by their respective provers. Every distinct prover set requires its own program deployment and initialization before use; prover configuration has no setter or close instruction. Details and account ordering: README's Aggregator-Prover section.
...
- **polymer-prover** — Polymer-backed prover, pull-based unlike hyper-prover: nobody calls us, a relayer loads a Polymer proof into Polymer's program (`create_accounts` → `load_proof`) and then calls our permissionless `validate`, which CPIs Polymer's `validate_event` and reads the `["result", authority]` account it writes **in the same instruction** (freshness), then mirrors the Solidity `PolymerProver.validate` checks (whitelisted emitter, `IntentFulfilledFromSource` selector, topic 1 == `CHAIN_ID`, payload destination == Polymer's `chain_id`) and creates `Proof` PDAs idempotently (same proof no-op, disagreeing proof `IntentAlreadyProven`). `prove` is gated to `dispatcher_pda(&polymer_prover::ID)` and emits one `Prove: program: <id>, <hex>` log per intent (hex = source u64 ‖ destination u64 ‖ intent hash ‖ claimant) for the EVM `PolymerProver.validateSolana` to parse — that string is a shared ABI. Polymer's Prove API hands back only the `<hex>` payload (it strips the whole `Prove: program: <id>, ` head, seen on devnet), so the EVM side binds the program through `validateSolLogs`' authenticated `programID`; it accepts both forms. `polymer.rs` hand-mirrors Polymer's IDs (mainnet `CdvSq48Q…`, devnet `FtdxWoZX…`, feature-gated), PDAs, discriminators and result layout the way `hyperlane.rs` mirrors the mailbox; Polymer's public docs describe an older interface, their repo source (v1.0.4) is authoritative. Callers set a 1.4M CU limit on `validate`. Operational notes: (1) `validate` has no partial-batch mode — every pair in the event needs its Proof PDA in one transaction — so keep EVM `Inbox.prove` batches destined for Solana at or below `MAX_INTENTS_PER_PROVE` (24), the same cap as the outbound direction, and deliver it with a v0 transaction plus an address lookup table — a legacy transaction tops out at 23 pairs (`MAX_PAIRS_PER_VALIDATE_LEGACY_TX`). The real ceiling is the 64-entry instruction trace: 30 fresh pairs, and an adversary who pre-funds 7 Proof PDAs pushes a 24-pair batch over it; account locks (54), CU (~252k of 1.4M) and Polymer's `unindexed_data` cap (~45) never bind. `MaxInstructionTraceLengthExceeded` means re-prove with a smaller batch — the intent is not lost, only the Polymer proof. Full derivation lives in `validate.rs::mark_intent_hashes_proven`'s doc comment; keep validate.rs, CLAUDE.md and spec 3.4 in sync. (2) The 10 KB log buffer `MAX_INTENTS_PER_PROVE` defends is per *transaction* while the cap is per instruction, so submit `portal::prove` as the only log-emitting instruction in its transaction (a full 24-intent batch overruns the 1232-byte packet — 24 FulfillMarker accounts plus 24 hashes — so send it as a v0 transaction with the markers in an address lookup table, as for `validate`), then count the `Program log: Prove: program: <polymer_prover id>, ` lines in `meta.logMessages` against the hashes sent and resubmit any shortfall — `prove` writes no state, so the retry is safe.
...
- **`prover::prove` CPI** (`packages/eco-svm-std/src/prover.rs`) builds local-prover's fixed six-account layout, signing `caller` via `caller_seeds`. Flash-Fulfiller uses its prover-scoped `prove_authority_pda`. Portal's `prove` assembles a variable account list and signs its prover-scoped `dispatcher` to a concrete prover. Aggregator's source-chain `aggregate` is called separately, not through Portal's `prove` dispatch.
...
Per-program contexts live next to `mod.rs`: `portal_context.rs`, `hyper_prover_context.rs`, `local_prover_context.rs`, `aggregator_prover_context.rs`, `flash_fulfiller_context.rs`, `hyperlane_context.rs`, `proof_helper_context.rs`, `polymer_prover_context.rs`. They host the per-instruction builders that integration tests call.
...
Aggregator declares `IntentProven` locally so Anchor includes the event and its type in the generated IDL. Keep its discriminator and Borsh layout identical to `eco_svm_std::prover::IntentProven`; `aggregator_prover.rs` checks the generated `target/idl/aggregator_prover.json` and emitted CPI bytes. Its bridge settlement tests execute Hyperlane mailbox delivery or Polymer validation before aggregation and Portal withdrawal for both token programs; bridge verification is mocked, while the concrete Eco prover programs run their compiled instructions.
